    ...
    <div class="row">
      <section class="search col-sm-12 mb-2">
        ...
        <div class="d-flex justify-content-start align-items-center">
          <a class="btn btn-primary mr-3"  href="<%=request.getContextPath()%>/student?type=toAdd">新增</a>
          <form method="post" class="form-inline mb-0" action="<%=request.getContextPath()%>/student?type=toManage">
            <input  class="form-control" type="text" name="no" placeholder="按学号模糊搜索" value="<%= tableResult.getSearchWord()%>"/>
      ...
      <section class="col-sm-12">
        <table class="table table-bordered table-hover table-striped">
          ...
          <tr>
            ...
            <td><%= "M".equals(oneDO.getGender()) ? "男" : "女" %>
            </td>
            <td><%= oneDO.getDept()%>
            </td>
            <td><%= oneDO.getGrade()%>
            ...
            <td><%= oneDO.getDormitoryNo()%>
            </td>
            <td>
              <a href="<%=request.getContextPath()%>/student?type=delete&id=<%= oneDO.getId()%>">删除</a>
              <a href="<%=request.getContextPath()%>/student?type=toUpdate&id=<%= oneDO.getId()%>&pageNow=<%=tableResult.getPageNow()%>">更新</a>
            </td>
          </tr>
          <%
            }
