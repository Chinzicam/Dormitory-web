  ...
  <div class="container">
    ...
    <div class="row">
      <section class="search col-sm-12 mb-2">
        <%
          TableResult<BuildingDO> tableResult = (TableResult) request.getAttribute("tableResult");
        %>
        <div class="d-flex justify-content-start align-items-center">
          <a class="btn btn-primary mr-3"
             href="<%=request.getContextPath()%>/building?type=toAdd">
            新增
          </a>
          <form method="post" class="form-inline mb-0" action="<%=request.getContextPath()%>/building?type=toManage">
            <input  class="form-control" type="text" name="no" placeholder="按宿舍楼编号模糊搜索" value="<%= tableResult.getSearchWord()%>"/>
      ...
      <section class="col-sm-12">
        <table class="table table-bordered table-hover table-striped">
          ...
          <tbody>
          <%
            List<BuildingDO> data = tableResult.getData();
            for (BuildingDO oneDO : data) {
          %>
          <tr>
            <td><%= oneDO.getId()%>
            </td>
            <td><%= "M".equals(oneDO.getType()) ? "男生宿舍" : "女生宿舍"%>
            </td>
            <td><%= oneDO.getNo()%>
            </td>
            <td><%= oneDO.getCompletedDate()%>
            </td>
            <td>
              <a href="<%=request.getContextPath()%>/building?type=delete&id=<%= oneDO.getId()%>">删除</a>
              <a href="<%=request.getContextPath()%>/building?type=toUpdate&id=<%= oneDO.getId()%>&pageNow=<%=tableResult.getPageNow()%>">更新</a>
            </td>
          </tr>
          <%
            }
